...
object GlauxHandlers {
  ...
  def tf[T <: Tensor: TensorBuilder: InputDimensionHandler]: Handler[T] = new HandlerTrait[T] {
    def read(bson: BSONDocument): T = (
      bson.getAs[T#Dimensionality]("dimension").get,
  ...
  implicit def fcf[T <: Tensor: TensorBuilder: InputDimensionHandler] = (
    ...
    field[String]("id")
  )(FullyConnected.apply[T], unlift(FullyConnected.unapply[T]))
  ...
  implicit val hiddenLayerH = polymorphic[HiddenLayer](
    new Partial[HiddenLayer, Convolution],
  ...
  implicit val lossLayerH = polymorphic[LossLayer](
    new Partial[LossLayer, Regression],
  ...
  implicit def temporalStateH[Input <: Tensor: TensorBuilder: Handler] =
    (field[Input]("readings") ~
      field[Time]("time"))(TemporalState.apply[Input], unlift(TemporalState.unapply[Input]))
  ...
  implicit def learnerIterationH[T <: DeepMindQLearner](learner: T)(
    implicit
    nh: Handler[learner.Net], trh: Handler[learner.TrainingResult], transitionH: Handler[Transition[learner.Input]], sh: Handler[learner.State]
  ) = {
  ...
  def sessionH[Iteration <: QLearner#IterationLike: Handler, Context <: Policy.DecisionContext: Handler]: Handler[Session[Iteration, Context]] = {
