...
class SimplifiedDeepMindQLearnerSpec extends Specification {
  ...
  "initial iterations" >> {
    ...
    val thirdIter = learner.iterate(init, Observation(
      lastAction = 1,
      reward = 1,
    ...
    val fourth = learner.iterate(thirdIter, Observation(
      lastAction = 1,
      reward = 1,
      recentHistory = Seq(
        TemporalState(RowVector(4, 3), start.plusMinutes(3)),
    ...
    val fifth = learner.iterate(fourth, Observation(
      lastAction = 3,
      reward = 2,
      recentHistory = Seq(
        TemporalState(RowVector(5, 4), start.plusMinutes(5)),
